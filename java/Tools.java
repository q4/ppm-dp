...
public class Tools {
  ...
  /** Computes the inverse logit of argument <var>x</var>.
    * This function is also known as the <em>logistic function</em>.
    * Definition: invlogit(x) = 1 / (exp(-x) + 1). */
  public static double invlogit(double x) {
    return 1.0 / (Math.exp(-x)+1.0);
  }

  /** Multivariate logit (inverse softmax).
    * @param pp vector of probability masses that sum to one.
    * @return multivariate logit (inverse softmax)
    * @see #invlogit(ArrayList) */
  public static ArrayList<Double> logit(ArrayList<Double> pp) {
    int size = pp.size();
    ArrayList<Double> logit = new ArrayList<Double>(size-1);
    double plast = pp.get(size-1);
    double llast = Math.log(plast);
    for (int k=0; k<size-1; k++) {
      logit.add(Math.log(pp.get(k)) - llast);
    }
    return logit;
  }

  /** Softmax (inverse of multivariate logit).
    * @param pp vector containing multivariate logit
    * @return vector of probability masses that sum to one
    * @see #logit(ArrayList) */
  public static ArrayList<Double> invlogit(ArrayList<Double> logit) {
    int size = logit.size();
    ArrayList<Double> pp = new ArrayList<Double>(size+1);
    double sum = 0.0;
    for (int k=0; k<size; k++) {
      double r = Math.exp(logit.get(k));
      pp.add(r);
      sum += r;
    }
    pp.add(1.0);
  ...
  */
  ...
  /** Computes the least common multiple.
    * Assumes <code>a</code> and <code>b</code> are strictly positive.
    * Uses <code>gcd(a,b)</code>.
    * @see #gcd(int,int) */
  public static int lcm(int a, int b) {
    return (a < b) ? a*(b/gcd(a,b)) : (b*(a/gcd(b,a)));
  }
  ...
  /** Returns the floor of the square root of x, treated as unsigned.
    * See http://www.codecodex.com/wiki/Calculate_an_integer_square_root#Java */
